...
export class OrderFormComponent implements OnInit {
  ...
  private initializeOrderForm(): void {
    this.orderForm = new FormGroup({
      ...
      internalNumberYearPart: new FormControl(new Date().getFullYear(), [
        ...
        Validators.min(2020),
        Validators.max(9999),
      ]), // Champ d'année initialisé avec l'année en cours
  ...
  assembleInternalNumber(): string {
    ...
    const abbr = (
      this.orderForm.get("internalNumberAbbrPart").value || ""
    ).toUpperCase();
    const numericPart =
      this.orderForm.get("internalNumberNumericPart").value || "";
    ...
    if (!abbr || !numericPart || !year) {
      ...
      console.error(
        "Numéro interne incomplet : abbr, numericPart ou year manquant."
      );
      return "";
    }
    ...
    const finalInternalNumber = suffix
      ? `${internalNumber}-${suffix}`
      : internalNumber;
  ...
  onSubmit(): void {
    ...
    if (this.orderForm.valid) {
      ...
      this.toastr.error(
        "Certains champs du formulaire sont invalides. Veuillez les corriger."
      );
    }
  }
