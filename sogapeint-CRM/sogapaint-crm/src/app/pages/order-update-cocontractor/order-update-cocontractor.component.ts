...
@Component({
  ...
  templateUrl: "./order-update-cocontractor.component.html",
  styleUrls: [
    "./order-update-cocontractor.component.scss",
    "../order-detail/order-detail.component.scss",
  ],
})
export class OrderUpdateCocontractorComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.replaceUsersByTheirCompanies();
    ...
    if (this.contractId) {
      ...
      this.contractService
        ...
        .subscribe((contract) => {
          ...
          // convertit occupied (boolean) en strin 'Oui' ou 'Non'
          contract.occupied = contract.occupied ? "Oui" : "Non";
  ...
  replaceUsersByTheirCompanies(): void {
    ...
    this.orderUpdateForm.get('external_contributor').valueChanges.subscribe((userId: string) => {
      this.getCompanyNameFromUserId(userId).subscribe(companyName => {
        console.log('external contributor company name:', companyName);
        this.orderUpdateForm.patchValue({ external_contributor: companyName }, { emitEvent: false });
      });
    });

    // subcontractor
    this.orderUpdateForm.get('subcontractor').valueChanges.subscribe((userId: string) => {
      this.getCompanyNameFromUserId(userId).subscribe(companyName => {
        console.log('subcontractor company name:', companyName);
        this.orderUpdateForm.patchValue({ subcontractor: companyName }, { emitEvent: false });
      });
    });
  }
