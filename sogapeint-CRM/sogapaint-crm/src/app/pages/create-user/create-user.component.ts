...
export class CreateUserComponent implements OnInit {
  ...
  checkPasswordStrength(password: string) {
    ...
    this.passwordFeedback =
      results.feedback?.suggestions?.join(", ") ?? "No suggestions available";
  }
  ...
  async onCompanyInputBlur(event: any): Promise<void> {
    ...
    if (inputValue && companyExists === false) {
      ...
      this.companyService
        .createCompany({ normalized_name: normalizedInputValue })
        .subscribe({
          next: (data: any) => {
          ...
          error: (error) => {
            ...
            this.errorMessage =
              "Erreur lors de la création de l'entreprise : " + error;
          },
          complete: () => {
